...
def receive_upload(handler):
    ...
    for field in fields:
        ...
        if filename:
            ...
            if name_conflict:
                return_obj = { 'renamed_fname': os.path.basename(destination) }
                result = (http.HTTPStatus.OK, 'Some filename(s) changed due to name conflict', return_obj)
            else:
                result = (http.HTTPStatus.NO_CONTENT, 'Files accepted')
...
def delete_upload(filename):
    ...
    if os.path.exists(destination) and os.path.isfile(destination):
        os.remove(destination)
        result = (http.HTTPStatus.NO_CONTENT, 'File successfully deleted')
...
class SimpleHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path in ['/upload', '/upload/validateToken']:
            response_json_bytes = None
            if self.path == '/upload/validateToken':
                result = validate_token(self)
            elif self.path == '/upload':
                result = receive_upload(self)
                if len(result) == 3:
                    response_json_bytes = json.dumps(result[2]).encode()
            self.send_response(result[0], result[1])
            self.end_headers()
            if response_json_bytes:
                self.wfile.write(response_json_bytes)  # @NOTE: this writes the json response body.
        elif self.path in ['/test_500_error']:
            self.send_error(http.HTTPStatus.INTERNAL_SERVER_ERROR, 'This is a sample error message')
            self.end_headers()
        else:
    ...
    def do_DELETE(self):
        ...
        if parsed_url.path == '/upload':
            result = delete_upload(parse_qs(parsed_url.query)['fname'][0])
